# Swagger 文档更新指南

本文档说明如何在更新代码后同步更新 Swagger API 文档。

## 📋 目录
1. [快速开始](#快速开始)
2. [更新流程](#更新流程)
3. [常见场景](#常见场景)
4. [验证步骤](#验证步骤)
5. [故障排除](#故障排除)

---

## 快速开始

### 项目结构
```
docs/
├── docs.go           ← Swagger 文档定义（自动生成）
├── swagger.yaml      ← OpenAPI 3.0 YAML 格式
└── swagger.json      ← OpenAPI 3.0 JSON 格式
```

### 访问 Swagger UI
```
http://localhost:8080/swagger/index.html
```

---

## 更新流程

### 步骤 1: 更新 `docs/swagger.yaml`

当你添加或修改 API 端点时，首先更新 YAML 文件：

#### 1.1 添加新的 API 端点

在 `docs/swagger.yaml` 的 `paths` 部分添加新端点：

```yaml
paths:
  /api/v1/new-endpoint:
    post:
      tags:
        - 功能分类
      summary: 简短描述
      description: 详细描述
      requestBody:
        required: true
        content:
          application/json:
            schema:
              type: object
              required:
                - field1
              properties:
                field1:
                  type: string
                  example: "example_value"
      responses:
        '200':
          description: 成功响应
        '400':
          description: 请求参数错误
        '500':
          description: 服务器内部错误
```

#### 1.2 修改现有端点

找到对应的端点，更新其定义：

```yaml
paths:
  /api/v1/rooms:
    post:
      # 更新 summary、description、parameters 等
      summary: 更新后的描述
      # ...
```

#### 1.3 添加新的数据模型

在 `components/schemas` 部分添加新的数据结构：

```yaml
components:
  schemas:
    NewModel:
      type: object
      properties:
        id:
          type: string
          example: "123"
        name:
          type: string
          example: "example"
```

---

### 步骤 2: 更新 `docs/swagger.json`

`swagger.json` 是 `swagger.yaml` 的 JSON 格式版本。

#### 2.1 自动转换方式（推荐）

使用在线工具或命令行工具转换：

**方式 1: 使用在线工具**
- 访问 https://www.convertjson.com/yaml-to-json.html
- 复制 `swagger.yaml` 内容
- 粘贴到转换工具
- 复制转换后的 JSON 到 `swagger.json`

**方式 2: 使用命令行工具**
```bash
# 安装 yq（YAML 处理工具）
brew install yq  # macOS
# 或
apt-get install yq  # Linux

# 转换 YAML 到 JSON
yq eval -o=json docs/swagger.yaml > docs/swagger.json
```

**方式 3: 使用 Python**
```bash
python3 << 'EOF'
import yaml
import json

with open('docs/swagger.yaml', 'r', encoding='utf-8') as f:
    yaml_data = yaml.safe_load(f)

with open('docs/swagger.json', 'w', encoding='utf-8') as f:
    json.dump(yaml_data, f, ensure_ascii=False, indent=2)

print("✅ swagger.json 已更新")
EOF
```

#### 2.2 手动更新方式

如果只是小改动，可以直接编辑 `swagger.json`：

```json
{
  "openapi": "3.0.0",
  "info": {
    "title": "TgoCall Server API",
    "version": "1.0.0"
  },
  "paths": {
    "/api/v1/new-endpoint": {
      "post": {
        "tags": ["功能分类"],
        "summary": "简短描述",
        "requestBody": {
          "required": true,
          "content": {
            "application/json": {
              "schema": {
                "type": "object",
                "properties": {
                  "field1": {"type": "string"}
                }
              }
            }
          }
        },
        "responses": {
          "200": {"description": "成功"}
        }
      }
    }
  }
}
```

---

### 步骤 3: 更新 `docs/docs.go`

`docs.go` 是 swaggo 库使用的文件，包含 Swagger 文档的完整定义。

#### 3.1 更新 docTemplate

编辑 `docs/docs.go` 中的 `docTemplate` 常量，将 `swagger.json` 的内容复制进去：

```go
const docTemplate = `{
  "openapi": "3.0.0",
  "info": {
    "title": "{{.Title}}",
    "description": "{{escape .Description}}",
    "version": "{{.Version}}"
  },
  "paths": {
    "/api/v1/new-endpoint": {
      "post": {
        "tags": ["功能分类"],
        "summary": "简短描述",
        // ... 完整的 JSON 内容
      }
    }
  }
}`
```

**注意事项：**
- 保留 `{{.Title}}`、`{{.Description}}`、`{{.Version}}` 等模板变量
- 使用 `{{escape .Description}}` 来转义特殊字符
- JSON 必须是有效的格式

#### 3.2 快速更新脚本

创建一个脚本自动更新 `docs.go`：

```bash
#!/bin/bash
# 文件: update-swagger-docs.sh

# 读取 swagger.json
SWAGGER_JSON=$(cat docs/swagger.json)

# 创建新的 docs.go
cat > docs/docs.go << 'EOF'
// Package docs Code generated by swaggo/swag. DO NOT EDIT
package docs

import "github.com/swaggo/swag"

const docTemplate = `
EOF

# 添加 JSON 内容（需要转义反引号）
echo "$SWAGGER_JSON" | sed 's/`/\\`/g' >> docs/docs.go

cat >> docs/docs.go << 'EOF'
`

// SwaggerInfo holds exported Swagger Info so clients can modify it
var SwaggerInfo = &swag.Spec{
	Version:          "1.0.0",
	Host:             "localhost:8080",
	BasePath:         "/api/v1",
	Schemes:          []string{},
	Title:            "TgoCall Server API",
	Description:      "基于 LiveKit 的音视频服务 API",
	InfoInstanceName: "swagger",
	SwaggerTemplate:  docTemplate,
	LeftDelim:        "{{",
	RightDelim:       "}}",
}

func init() {
	swag.Register("swagger", SwaggerInfo)
}
EOF

echo "✅ docs.go 已更新"
```

使用方式：
```bash
chmod +x update-swagger-docs.sh
./update-swagger-docs.sh
```

---

## 常见场景

### 场景 1: 添加新的 API 端点

**步骤：**

1. **更新 `docs/swagger.yaml`**
   ```yaml
   paths:
     /api/v1/users/{user_id}:
       get:
         tags:
           - 用户管理
         summary: 获取用户信息
         parameters:
           - name: user_id
             in: path
             required: true
             schema:
               type: string
         responses:
           '200':
             description: 成功获取用户信息
           '404':
             description: 用户不存在
   ```

2. **转换为 JSON**
   ```bash
   yq eval -o=json docs/swagger.yaml > docs/swagger.json
   ```

3. **更新 `docs/docs.go`**
   - 复制 `swagger.json` 的内容
   - 粘贴到 `docs.go` 的 `docTemplate` 中

4. **构建并测试**
   ```bash
   go build -o tgo-call-server
   ./tgo-call-server
   # 访问 http://localhost:8080/swagger/index.html
   ```

---

### 场景 2: 修改现有端点的参数

**步骤：**

1. **在 `docs/swagger.yaml` 中找到对应端点**
   ```yaml
   /api/v1/rooms:
     post:
       requestBody:
         content:
           application/json:
             schema:
               properties:
                 new_field:  # 添加新字段
                   type: string
   ```

2. **同步到 `swagger.json` 和 `docs.go`**
   - 重复上面的转换步骤

---

### 场景 3: 添加新的数据模型

**步骤：**

1. **在 `docs/swagger.yaml` 的 `components/schemas` 中添加**
   ```yaml
   components:
     schemas:
       User:
         type: object
         properties:
           id:
             type: string
           name:
             type: string
           email:
             type: string
   ```

2. **在端点中引用**
   ```yaml
   responses:
     '200':
       description: 成功
       content:
         application/json:
           schema:
             $ref: '#/components/schemas/User'
   ```

3. **同步到 JSON 和 `docs.go`**

---

## 验证步骤

### 验证 1: 检查 YAML 语法

```bash
# 使用在线工具验证
# https://www.yamllint.com/

# 或使用命令行
yamllint docs/swagger.yaml
```

### 验证 2: 检查 JSON 语法

```bash
# 验证 JSON 格式
python3 -m json.tool docs/swagger.json > /dev/null && echo "✅ JSON 格式正确"

# 或使用 jq
jq empty docs/swagger.json && echo "✅ JSON 格式正确"
```

### 验证 3: 构建项目

```bash
go build -o tgo-call-server
# 如果构建成功，说明 docs.go 格式正确
```

### 验证 4: 启动服务并访问 Swagger UI

```bash
./tgo-call-server &
sleep 3

# 检查 Swagger UI 是否可访问
curl -s http://localhost:8080/swagger/index.html | head -20

# 检查 swagger.json 端点
curl -s http://localhost:8080/api/docs/swagger.json | jq '.paths | keys'

# 停止服务
pkill -f tgo-call-server
```

### 验证 5: 在浏览器中查看

1. 启动服务：`./tgo-call-server`
2. 打开浏览器：`http://localhost:8080/swagger/index.html`
3. 检查所有 API 端点是否都显示了
4. 尝试展开某个端点，查看参数和响应

---

## 故障排除

### 问题 1: Swagger UI 显示 404

**原因：** 服务未启动或路由配置错误

**解决方案：**
```bash
# 检查服务是否运行
ps aux | grep tgo-call-server

# 检查路由配置
grep -n "swagger" internal/router/router.go

# 重新构建
go build -o tgo-call-server
./tgo-call-server
```

---

### 问题 2: Swagger UI 只显示部分 API

**原因：** `docs.go` 中的 `docTemplate` 不完整

**解决方案：**
1. 检查 `docs/swagger.json` 是否包含所有端点
   ```bash
   cat docs/swagger.json | grep -o '"/api/v1/[^"]*"' | sort -u
   ```

2. 确保 `docs.go` 中的 `docTemplate` 包含完整的 JSON
   ```bash
   grep -c "paths" docs/docs.go
   ```

3. 重新更新 `docs.go`

---

### 问题 3: JSON 格式错误

**原因：** 手动编辑时引入了语法错误

**解决方案：**
```bash
# 验证 JSON
python3 -m json.tool docs/swagger.json

# 如果有错误，使用在线工具修复
# https://jsonlint.com/

# 或使用 jq 查看错误位置
jq . docs/swagger.json
```

---

### 问题 4: YAML 中文字符显示错误

**原因：** 文件编码问题

**解决方案：**
```bash
# 检查文件编码
file docs/swagger.yaml

# 转换为 UTF-8
iconv -f GBK -t UTF-8 docs/swagger.yaml > docs/swagger.yaml.tmp
mv docs/swagger.yaml.tmp docs/swagger.yaml
```

---

## 完整更新流程总结

```bash
# 1. 编辑 swagger.yaml
vim docs/swagger.yaml

# 2. 验证 YAML 语法
yamllint docs/swagger.yaml

# 3. 转换为 JSON
yq eval -o=json docs/swagger.yaml > docs/swagger.json

# 4. 验证 JSON 语法
python3 -m json.tool docs/swagger.json > /dev/null

# 5. 更新 docs.go（使用脚本或手动）
./update-swagger-docs.sh

# 6. 构建项目
go build -o tgo-call-server

# 7. 启动服务测试
./tgo-call-server &
sleep 3

# 8. 验证 Swagger UI
curl -s http://localhost:8080/swagger/index.html | head -20

# 9. 停止服务
pkill -f tgo-call-server

# 10. 提交代码
git add docs/
git commit -m "docs: update swagger documentation"
```

---

## 快速参考

| 文件 | 用途 | 何时更新 |
|------|------|--------|
| `docs/swagger.yaml` | OpenAPI 3.0 YAML 格式 | 添加/修改 API 时首先更新 |
| `docs/swagger.json` | OpenAPI 3.0 JSON 格式 | 从 YAML 转换得到 |
| `docs/docs.go` | swaggo 库使用的定义 | 从 JSON 复制内容 |

---

## 相关资源

- [OpenAPI 3.0 规范](https://spec.openapis.org/oas/v3.0.3)
- [Swagger 编辑器](https://editor.swagger.io/)
- [swaggo 文档](https://github.com/swaggo/swag)
- [YAML 验证工具](https://www.yamllint.com/)
- [JSON 验证工具](https://jsonlint.com/)

---

## 常见问题 (FAQ)

**Q: 我应该先更新哪个文件？**
A: 始终先更新 `swagger.yaml`，然后转换为 JSON，最后更新 `docs.go`。

**Q: 可以只更新 `docs.go` 吗？**
A: 不建议。应该保持三个文件同步，这样便于维护和版本控制。

**Q: 如何自动化这个过程？**
A: 使用提供的 `update-swagger-docs.sh` 脚本，或集成到 CI/CD 流程中。

**Q: 更新后需要重启服务吗？**
A: 需要重新构建（`go build`）并重启服务。

**Q: 如何在 Git 中跟踪这些文件？**
A: 将所有三个文件都提交到 Git，这样可以追踪文档的变化历史。


